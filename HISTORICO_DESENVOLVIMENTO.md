...
- ✅ **CP-14** — Formato de estaca (circular/quadrada) + dimensão livre + alerta A6: cadastro passa a ter FORMATO de seção (pré-moldada: circular ou quadrada) e DIMENSÃO em campo numérico livre (cm), substituindo a lista fixa de diâmetros. ENGINE: diff mínimo retrocompatível e pré-validado (AV_F1_F2_fn ganha 3º parâmetro B_m; `_calcularGenerico` aceita opcoes.area_ponta_m2/perimetro_m — quadrada Ap=L², U=4·L — senão deriva do diâmetro; B_m=dimensaoTransversal_m??D_m). Chamadas legadas produzem memorial BYTE A BYTE idêntico; 216 testes verdes sem alteração. MODELO: estaca.formato + estaca.dimensao_m, com diametro_m mantido como ESPELHO (retrocompat. de JSON, desenho e engine); migração automática de obras antigas. UI Aba 5: campo livre + seletor de formato só na pré-moldada; A_p/U ao vivo. ALERTA A6 (dimensão fora de 15–120 cm) preenche a lacuna histórica da numeração — único alerta sobre ESTACA — exibido no painel da Aba 4, na Aba 5 (inline+painel) e no JSON de auditoria; NÃO bloqueia o cálculo. CARGA ESTRUTURAL: lado_cm como chave equivalente da tabela (conservador: L²>πL²/4); override do fabricante tem precedência; sem entrada → null explícito (nada inventado). GEOMETRIA (A_p e U) visível em 5 locais (modal, Aba 6, PDFs, XLSX, auditoria.geometriaSecao). Validação: 32,84 · 45 · 12 · build · 12 checks sintéticos (geometria, F1, razão 4/π, override, limites A6)
- ✅ **CP-15** — Diagrama de transferência de carga estaca-solo (AOKI 1979): botão "📉 Transferência de carga" por método (DQ/AV) no resumo da Aba 6 (modo Envoltória) abre modal em tela cheia com 3 painéis em eixo de profundidade comum — desenho da estaca, esforço normal N(z) e tensão axial σ(z)=N/Ap. ENGINE INTOCADA: tudo derivado do memorial (PL=Ql_total_kN — exclui o bulbo; PP=Rp_final_kN; PR=Rrup_kN; Ap respeita o formato do CP-14). 3 cenários de carga no topo (Ruptura=R_rup; Carga prevista=a cadastrada; Prevista×FSg) e 2 modelos de AOKI nos cenários de trabalho (A: resistência local, ponto B quando P<PL; B: redistribuição proporcional, com B≡A em P≥PL — decisão validada). Ponta de trabalho=max(P−PL,0); ruptura usa PP. Cota DUPLA (absoluta│relativa), valores metro a metro, identificação da estaca em linha simples dentro do SVG, σ_topo sem corte, scroll para estacas longas. Aviso simplificado AOKI/NBR 16903 sempre visível. Nova suíte test-transferencia.mjs (33 asserções); 3 testes canônicos verdes
- ✅ **CP-15c/d** — Correções pós-revisão visual do CP-15: (1) CARGA ESTRUTURAL comparada no MESMO estado-limite (correção conceitual do usuário) — a estrutural da tabela é admissível (com FS); cenário "Carga prevista" (serviço) compara vs C_adm, cenários "Prevista×FS" e "Ruptura" (último) comparam vs C_adm×FSg; nunca altera o traçado, só referência; (2) FUSTE ACIMA DAS SONDAGENS (causa-raiz do NaN da E-04): camadas acima do topo das sondagens vinham com Ql indefinido e contaminavam a série — corrigido com cotaTopoSolo (atrito nulo no aterro → N/σ constantes até o topo do solo, depois decaem, conforme indicado pelo usuário), linha "topo do solo" no desenho, 3 testes novos; (3) renderização DEFENSIVA (descarta pontos não-finitos; aviso se série vazia — a tela nunca mostra NaN). Manual (nova subseção 5.6.1 + imagem 11 do diagrama), README, NOTAS_TECNICAS e STATUS atualizados. Próximo (CP-16): tensões máximas admissíveis do material por norma no diagrama. Validação: 32,84 · 45 · 12 · transferência 33 · build (você está aqui)

- ✅ **CP-16** — Tensão admissível estrutural (Tabela 1.10) + carga estrutural por hierarquia + alerta A-11 + linha-limite no diagrama: a carga estrutural admissível passa a ser σₑ×A_seção (Opção A), com σₑ da nova Tabela 1.10 editável (hélice 6, escavada a seco 5, escavada c/ fluido 6, pré-moldada 11, raiz 12 MPa; metálica 120 → CP-17). ENGINE com 2 adições ADITIVAS (tabela tensaoAdmissivel_MPa + util.cargaEstruturalNorma_tf); tabela de carga do CP-14 mantida como fallback morto → 216 testes externos e regressão 32,84 intactos. HIERARQUIA do valor efetivo: override → catálogo comercial (kN÷10, valores redondos) → cálculo σₑ×A, usado como limite estrutural em TODOS os cálculos (via cargaEstrutural_tf_override já existente). CATÁLOGOS das tabelas comerciais: hélice contínua, escavada a seco/trado, escavada c/ fluido/estação, pré-moldada circular vibrada, pré-moldada quadrada (Tab 2.2), raiz (limite superior). UI: nova seção da Tabela 1.10 no editor de coeficientes; campo de carga estrutural com botões clicáveis "usar catálogo/usar norma" + entrada manual; σ grego. ALERTA A-11 (carga em uso > σₑ×A, informando o valor de norma; Abas 4/5 + auditoria; não bloqueia). DIAGRAMA: linha vertical em σ=σₑ (serviço) ou σₑ×FSg (último/ruptura), trecho de σ(z) acima do limite em vermelho com cruzamento interpolado. PROPAGAÇÃO: carga efetiva + A-11 em XLSX, PDFs e auditoria JSON. Validação: 32,84 · 45 · 12 · transferência 33 · build (você está aqui)

- ✅ **Hotfix (pós-CP-16)** — Error Boundary + A-11 na Aba 4: (a) estaca Ø10 ao abrir o diagrama de transferência causava tela branca que derrubava o app inteiro e perdia dados; criado src/components/ErrorBoundary.jsx aplicado ao redor do conteúdo das abas E do diagrama — exceção de render agora vira mensagem recuperável ("↩ Voltar e continuar") sem desmontar o app nem perder o estado em memória; (b) o alerta A-11 (carga estrutural acima da norma σₑ×A) não aparecia na Aba 4 porque construirAlertas não recebia os coeficientes — corrigido (recebe coeficientesCustomizados, gera o A-11 no padrão do A-6, índice passa os coefs, textos A1–A11)
- ✅ **CP-17** — Salvamento automático (autosave) em localStorage: a obra passa a ser salva automaticamente no navegador, com debounce de ~1,5 s após cada alteração (resolve a causa-raiz da perda de dados: recarregar/travar não perde mais o trabalho). RECUPERAÇÃO ao abrir PERGUNTA se deseja restaurar (modal RecuperacaoAutosave). Payload ENXUTO (entrada + UI, SEM resultadosCalculo, recalculados ao abrir a Aba 6) → leve vs limite ~5 MB. Botão "🆕 Novo" no Header (limpa estado+autosave). Indicador discreto de status (salvando/salvo/erro/off). ROBUSTO: try/catch em todo acesso ao localStorage → incognito/quota/bloqueio degradam para 'off' sem quebrar. VERSIONADO: recusa schema de versão diferente (evita crash ao atualizar o app); JSON corrompido ignorado. Limitação comunicada: local ao navegador/máquina, NÃO substitui o export JSON (backup forte). Novos: src/state/persistenciaObra.js (12 testes), src/components/RecuperacaoAutosave.jsx. Modificados: ObraProvider.jsx, Header.jsx, App.jsx, manual (5.7.1), README, STATUS, NOTAS. Próximo (CP-18): estaca metálica. Validação: 32,84 · 45 · 12 · transferência 33 · persistência 12 · build (você está aqui)
